const setup = require('../fetchRetry')
...
describe('fetchRetry', () => {
    ...
    it('resolves on >MAX_RETRIES', async () => {
        ...
        return new Promise((resolve, reject) => {
            server.listen(async () => {
                try {
                    const { port } = server.address()
                    const response = await retryFetch(`http://127.0.0.1:${port}`, {
                        retry: {
                            retries: 3,
                        },
                    })
    ...
    it("don't retry if the request was aborted after timeout", async () => {
        const timeout = 500
        ...
        return new Promise((resolve, reject) => {
            server.listen(async () => {
                try {
                    ...
                    expect(opts.onRetry).toHaveBeenCalledTimes(0)
                    resolve()
                } finally {
                    server.close()
        ...
        return new Promise((resolve, reject) => {
            ...
            server.listen(async () => {
                try {
                    ...
                    expect(opts.onRetry).toHaveBeenNthCalledWith(1, expect.any(ResponseError), opts)
